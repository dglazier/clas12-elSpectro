//clas12-elSpectro --ebeam 10.6 --seed 2132 --trig 10 --misc '--tslope=4 --flat=1' MesonEx_nKKPi_Flat_s_give_t_MKKPi.C
...
void MesonEx_nKKPi_Flat_s_give_t_MKKPi(C12Config config) {
  ...
  for(auto entry:*tokens) {
    TString sentry= entry->GetName();///get actual string
     if(sentry.Contains("MKK=")){ //look to see if misc overrides mass distribition Y
      massDistY = sentry;
      massDistY.ReplaceAll("MKK=","");
    }
    if(sentry.Contains("MKKPi=")){ //look to see if misc overrides mass distribition X
      massDistX = sentry;
      massDistX.ReplaceAll("MKKPi=","");
    }
  ...
  auto Y=static_cast<DecayingParticle*>(particle(9996,model(new PhaseSpaceDecay{{},{321,-321}})) );
